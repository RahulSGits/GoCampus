...
const AdminDashboard = () => {
  ...
  const handleSendAlert = async (e) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:5001' : '')}/api/notifications`, {
        method: 'POST',
  ...
  useEffect(() => {
    // Analytics Hydration
    fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:5001' : '')}/api/analytics`)
      .then(res => res.json())
      .then(data => {
        setStats([
    ...
    fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:5001' : '')}/api/buses`)
      .then(res => res.json())
      .then(data => {
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md">
            ...
            <form onSubmit={async (e) => {
              e.preventDefault();
              try {
                const res = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:5001' : '')}/api/buses`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md">
            ...
            <form onSubmit={async (e) => {
              e.preventDefault();
              try {
                const res = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:5001' : '')}/api/routes`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
